...
figure('Position', [100, 100, 800, 600]);
region_index = yq >= z_values(end - 3) & yq <= 0.10;
xq_valid = xq .* region_index;
yq_valid = yq .* region_index;
vq_valid = vq .* region_index;
contourf(xq_valid, yq_valid / H, vq_valid, 4, 'LineStyle', '--');
...
axis([(min(xv(xv > 0))) (max(xv(:))) z_values(end - 3)/H 0.1/H]); % limite y axis to water depth and ignore near wall region
